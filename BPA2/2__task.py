...
while True:
    ...
    mask = cv2.inRange(hsv,(0,127,196),(207,255,255))
    ...
    black = np.sum(mask[h//3: 2*(h//3), w//4: (w//4)*3])
    print(black)
    print("ind", ind)

    # cv2.circle(frame, ( w//4, h//3 ), 5, (255,0,255), -1)
    # cv2.circle(frame, ( (w//4)*3, (h//3)*2 ), 5, (255,0,255), -1)

    #cv2.rectangle(frame, (w//4, h//3), ((w//4)*3, (h//3)*2), (255,0,255), 3)

    # hist2 = np.sum(mask[0:w//2], axis=0)
    # maxStrInd2 = np.argmax(hist2)
    # ind2 = hist2[maxStrInd2]//255
    # print(ind2)
    # cv2.line(frame, (maxStrInd2, 0), (maxStrInd2, frame.shape[0]), 60, 4)

    # hist3 = np.sum(mask[w//2 : w], axis=0)
    # maxStrInd3 = np.argmax(hist3[0:w//2])
    # ind3 = hist3[maxStrInd3]//255
    # print(ind3)
    # cv2.line(frame, (maxStrInd3, 0), (maxStrInd3, frame.shape[0]), 60, 4)


    # 

    # black = np.sum(mask[high//2:high, 0:mask.shape[1]])
    # print(black)

    # black2= np.sum(mask[0:high, 0:mask.shape[1]])
    # print(black2)

    #if ind > 380 and ind < 410 and once and black < 50000:
    if ind > 250 and once and black == 0:
        print("Area:sguhguhuhtop")
        STATE = STOP
        once = False
    ...
    angle = int(95 + KP * err + KD * (err - last_err)) # EXPERIMENT 90 -> 85
    last_err = err
